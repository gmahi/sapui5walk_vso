		...
		onTheMasterPage : {
			...
			actions : {
				...
				iSortTheListOnName : function () {
					return this.iChooseASorter("sortButton", "Sort By <SalesOrder>");
				},
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("SalesOrder") === oObject.SalesOrder;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].SalesOrder;
									} else {
										sCurrentId = aItemsNotInTheList[0].SalesOrder;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("ZDEMO_C_SalesOrder_TX", {
										SalesOrder : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("SalesOrder"),
						title: oBindingContext.getProperty("SalesOrder")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<ZDEMO_C_SalesOrder_TX> (0)"}),
						success: function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("SalesOrder");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all ZDEMO_C_SalesOrder_TX to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<ZDEMO_C_SalesOrder_TX> (" + iExpectedLength + ")"}),
								success : function () {
